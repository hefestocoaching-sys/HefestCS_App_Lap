...
class TrainingPlanNotifier extends Notifier<TrainingPlanState> {
  ...
  Future<void> generatePlan({
    ...
    try {
      ...
      final structuredFlowError = _validateStructuredFlow(
        freshClient.training.extra,
      );
      if (structuredFlowError != null) {
        state = TrainingPlanState.blocked(reason: structuredFlowError);
        return;
      }
      ...
      // PASO 5 — Upsert: si ya existe un activePlanId (mismo ciclo), reemplazar
      // en lugar de acumular. Evita que trainingPlans crezca indefinidamente.
      final existingActivePlanId =
          freshClient.training.extra[TrainingExtraKeys.activePlanId] as String?;
      final List<TrainingPlanConfig> updatedPlans;
      if (existingActivePlanId != null && existingActivePlanId.isNotEmpty) {
        // Reemplazar: quitar el plan activo anterior + el mismo ID si ya existe
        updatedPlans = [
          ...freshClient.trainingPlans.where(
            (p) => p.id != existingActivePlanId && p.id != planConfig.id,
          ),
          planConfig,
        ];
      } else {
        // Primer plan: solo evitar duplicado de mismo ID
        updatedPlans = [
