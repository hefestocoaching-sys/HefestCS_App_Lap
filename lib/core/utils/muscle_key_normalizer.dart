...
///
/// EJEMPLOS:
/// - deltoide_anterior → delts_front (canónico)
...
Map<String, int> normalizeLegacyVopToCanonical(Map<dynamic, dynamic> raw) {
  ...
  final hasAnyDelt =
      tmp.containsKey('delts_front') ||
      tmp.containsKey('delts_lateral') ||
      tmp.containsKey('delts_rear');

  if (tmp.containsKey('shoulders')) {
    final shVal = tmp.remove('shoulders') ?? 0;
    if (hasAnyDelt) {
      tmp['delts_lateral'] = (tmp['delts_lateral'] ?? 0) + shVal;
    } else {
