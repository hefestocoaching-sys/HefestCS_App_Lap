...
class VolumeValidator {
  static const Map<String, Map<String, int>> _defaultLandmarksByLevel = {
  ...
  static Map<String, dynamic> validateProgram({
    ...
    final normalizedVolume = _normalizeVolumeMap(volumeByMuscle);

    // VALIDACIÓN 1: Cada músculo entre VME-MRV
    normalizedVolume.forEach((muscle, volume) {
    ...
    final totalVolume = normalizedVolume.values.fold(
      0,
      (sum, vol) => sum + vol,
    );
    if (totalVolume > 200) {
    ...
    final balanceIssues = _checkMuscleBalance(normalizedVolume);
    ...
    final requiredMuscles = [
      ...
      MuscleKey.deltsFront, // shoulders frontal
      MuscleKey.deltsLateral, // shoulders lateral
      MuscleKey.quads,
    ];
    final missingMuscles = requiredMuscles
        .where((m) => !normalizedVolume.containsKey(m))
        .toList();
    ...
    return {
      ...
      'muscles_validated': normalizedVolume.length,
    };
  }
  ...
  static Map<String, int> _getVolumeLandmarks(String muscle, String level) {
    final landmarksByMuscle = {
      'back': {
        ...
        'intermediate': {'vme': 12, 'mav': 20, 'mrv': 26},
        'advanced': {'vme': 14, 'mav': 22, 'mrv': 30},
      },
      'pectorals': {
    ...
    return byMuscle[safeLevel] ?? _defaultLandmarksByLevel[safeLevel]!;
  }
  ...
  static double calculateVolumeQualityScore({
    ...
    final normalizedVolume = _normalizeVolumeMap(volumeByMuscle);
    if (normalizedVolume.isEmpty) return 0.0;

    int totalMuscles = normalizedVolume.length;
